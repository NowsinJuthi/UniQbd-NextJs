...
const ViewPage = () => {
    ...
    const fetchOrder = async () => {
      try {
        const { data } = await axios.get(
          `https://uniqbd-nextjs-3.onrender.com/api/v1/order/${id}`,
  ...
  useEffect(() => {
    const fetchAllNotes = async () => {
      try {
        const { data } = await axios.get(
          `https://uniqbd-nextjs-3.onrender.com/api/v1/all-notes`,
          { withCredentials: true },
        );
  ...
  const updateStatus = async (orderId, status) => {
    try {
      await axios.put(
        `https://uniqbd-nextjs-3.onrender.com/api/v1/order/status/${orderId}`,
        { order_status: status },
        { withCredentials: true },
      );
  ...
  const sendNoteToCustomer = async () => {
    try {
      ...
      await axios.post(
        `https://uniqbd-nextjs-3.onrender.com/api/v1/send-notes-to-customer`,
        {
          orderId: id,
